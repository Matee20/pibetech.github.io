...
<body>
  ...
  $sql = "SELECT id, imagen, nombre, precio FROM oferta ORDER BY RAND()";
  $result = $conn->query($sql);

  // Verificamos si la consulta fue exitosa y se encontraron productos
  $productos = [];
  if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
      $productos[] = [
        "id" => $row["id"], // Guardamos el ID del producto
        "imagen" => $row["imagen"],
        "nombre" => $row["nombre"],
        "precio" => $row["precio"]
      ];
    ...
    <section class="cont">
      <?php foreach ($productos as $producto): ?>
        <div class="videojuego">
          <!-- Corregido: Agregamos el parámetro categoria -->
          <a href="../compra.php?id=<?php echo $producto['id']; ?>&categoria=oferta">
            <div class="producto">
              <img src="<?php echo $producto['imagen']; ?>" alt="<?php echo $producto['nombre']; ?>">
              <h2><?php echo $producto['nombre']; ?></h2>
              <p>$<?php echo number_format($producto['precio'], 2); ?></p> <!-- Formato de precio con decimales -->
